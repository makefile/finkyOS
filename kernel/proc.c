...
system_call sys_call_table[NR_SYS_CALL]={sys_sendrec,sys_write,sys_printx};			  
...
PUBLIC void schedule()
{
	...
	while (!greatest_ticks) {
		...
		//for (p = (PROCESS*)&FIRST_PROC; p <= (PROCESS*)&LAST_PROC; p++) {
...
PUBLIC int sys_sendrec(int function, int src_dest, MESSAGE* m, PROCESS* p)
{
	...
	assert((src_dest >= 0 && src_dest < NR_TASKS + NR_PROCS) ||
	       src_dest == ANY ||
	       src_dest == INTERRUPT);//确保src_dest在范围内
